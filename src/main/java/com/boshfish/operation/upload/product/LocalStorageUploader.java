package com.boshfish.operation.upload.product;
...
import com.boshfish.exception.NotSameFileExpectation;
import com.boshfish.exception.UploadGeneralException;
...
public class LocalStorageUploader extends Uploader {
    ...
    public List<UploadFile> upload(HttpServletRequest httpServletRequest, UploadFile uploadFile) {
        ...
        try {
            ...
            throw new UploadGeneralException("未包含文件上传域");
        } catch (NotSameFileExpectation notSameFileExpection) {
            notSameFileExpection.printStackTrace();
        }
        return saveUploadFileList;
    }

    private List<UploadFile> doUpload(StandardMultipartHttpServletRequest standardMultipartHttpServletRequest, String savePath, Iterator<String> iter, UploadFile uploadFile) throws IOException, NotSameFileExpectation {
        List<UploadFile> saveUploadFileList = new ArrayList<UploadFile>();
        ...
        if (isComplete) {
            ...
            if (StringUtils.isNotBlank(md5) && !md5.equals(uploadFile.getIdentifier())) {
                throw new NotSameFileExpectation();
            }
